...
1. Create `packages/linter/src/rules/<rule-name>.ts` exporting a `Rule` (interface in `packages/linter/src/types.ts`). Follow an existing rule as the pattern — `operation-tags.ts` is a small one. Reuse the walk helpers (`iterateOperations` in `openapi-walk.ts`, `childAt` in `util.ts`) where they fit.
...
3. The rule `name` is the user's config key in `oasis.config.jsonc` (`lint.rules`). Use kebab-case; the `structure/` prefix is reserved for structural validation of the OpenAPI schema itself.
