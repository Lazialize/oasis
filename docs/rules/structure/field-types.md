...
- A string `name` and an `in` set to one of `query`/`header`/`path`/`cookie` are required.
...
- `style` (if present) must be one of the values valid for the parameter's `in` (`matrix`/`label`/`simple` for `path`, `form`/`spaceDelimited`/`pipeDelimited`/`deepObject` for `query`, `simple` for `header`, `form` for `cookie`).
- `explode` (if present) must be a boolean.
- `allowEmptyValue`/`allowReserved` (if present) must be booleans and only apply to `in: query` parameters.
